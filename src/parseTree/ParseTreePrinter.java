...
public class ParseTreePrinter {
	...
	private void appendType(ParseNode node) {
		if(printLevel.printDecorations()) {
			result.append(node.getType().typeString() + " ");
		}
	}
	private void appendArrayOperation(ParseNode node) {
		if(printLevel.printDecorations()) {
			if (node instanceof ArrayOperatorNode) {
				ArrayOperatorNode arrayOperatorNode = (ArrayOperatorNode)node;
				result.append(arrayOperatorNode.getOperation() + " ");
			}
		}
	}
